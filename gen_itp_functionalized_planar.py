...
# For the sake of time, this script is just modified from the script genitp_functionalized_slit.py
# by not generating the verticle sheets.
...
parser.add_argument('-s', dest = 'sc', type = float, help = 'surface charge density')
...
def gen_circle(pstart, lx, ly, lz, myfile, gtype):
    ...
    if args.sc != None:
        ...
        surc = float(raw_input('Please input the surface charge density (C/m^2):\n'))
    e0 = 0.160217657
    ...
    #            myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'COH', 1, 'GPH', 'COH', atom_num, delta+0.1330, 12.011))
    #        else:
    ...
    for rept in range(2):
        for i, item in enumerate(vframe):
            ...
            if i in vfspot:
                fspot.append(atom_num)
                myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'COH', 1, 'GOH', 'COH', atom_num, (-1)**rept*delta+0.373, 12.011))
            else:
                myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'CG', 1, 'GOH', 'CG', atom_num, (-1)**rept*delta, 12.011))
        if gtype == 2:
            #fgroup = []
            #for rept in range(2):
            ...
            #        if i%2 == 0:
            #            myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'OG', 1, 'GPH', 'OG', atom_num, delta-0.5571, 15.9994))
            #        else:
            #            myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'HG', 1, 'GPH', 'HG', atom_num, delta+0.4241, 1.008))
            #for rept in range(2):
            for i, item in enumerate(vfframe):
                atom_num += 1
                fgroup.append(atom_num)
                if i%2 == 0:
                    myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'OG', 1, 'GOH', 'OG', atom_num, (-1)**rept*delta-0.789, 15.9994))
                else:
                    myfile.write('%5d%5s%5d%5s%5s%5d%12.7f%8.3f\n' %(atom_num, 'HG', 1, 'GOH', 'HG', atom_num, (-1)**rept*delta+0.416, 1.008))
...
unit = [6, 6] # Repeat unit for functional groups

# Lengths for the out circle
lx = 6.3
ly = 4.4
lz = 5.2
myfile = open('ohgraphene.itp','w')
myfile.write('[ moleculetype ]\n; molname   nrexcl\n')
myfile.write('GOH   3\n')
myfile.write('[ atoms ]\n')
myfile.write('; id type    res residu  at  cg  charge   mass\n')

fspot, fgroup, ny= gen_circle([0, 0], lx, ly, lz, myfile, 2)
