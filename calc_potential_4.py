...
# Description: This is a python script to calculate the electrical potential distribution along the channel for zigzag edge
...
#    exb_2 = 1.494
    ...
    exb_3 = float(args.exb[2])
    exb_4 = float(args.exb[3])
filename = args.filename
if not os.path.isfile(filename):
    sys.exit('Exit: not density profile named %s found in current directory!' %filename)


sigma1 = -0.1268*40*15/4.91902/5.11500/6.2415/2
sigma2 = -sigma1
sigma3 = sigma1
sigma4 = -sigma1
if args.sigma != None:
...
for seq, myrho in enumerate([rho, rho[::-1]]):
    ...
    phi_3 = np.zeros(len(z))
    phi_4 = np.zeros(len(z))
    ...
    for i, u in enumerate(z):
        if z[i] < exb_3:
            phi_3[i] = 0
        elif z[i] > bound[1]-exb_3:
            phi_3[i] = phi_3[i-1]
        else:
            phi_3[i] = -sigma3*(z[i]-exb_3)*const/0.160217657
    for i, u in enumerate(z):
        if z[i] < exb_4:
            phi_4[i] = 0
        elif z[i] > bound[1]-exb_4:
            phi_4[i] = phi_4[i-1]
        else:
            phi_4[i] = -(sigma4+(-1)**seq*sigma)*(z[i]-exb_4)*const/0.160217657
#    print sigma1+sigma2+(-1)**seq*sigma+sigma3+sigma4+(-1)**seq*sigma
