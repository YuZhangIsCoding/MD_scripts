...
parser.add_argument('-i', '--input', dest = 'filename', default = 'NumberDensity.txt', help = 'input file of number density')
...
for j in range(1, 4):
    ...
    for i in range(1, len(nlist)):
        if i == len(nlist)-1:
            ...
            #if np.trapz(numd[nlist[i-1]:nlist[i], j], numd[nlist[i-1]:nlist[i], 0]) == 0:
            #    zeff[j-1].append(0)
            #else:
                #zeff[j-1].append(np.trapz(numd[nlist[i-1]:nlist[i], j]*numd[nlist[i-1]:nlist[i], 0], numd[nlist[i-1]:nlist[i], 0])/np.trapz(numd[nlist[i-1]:nlist[i], 0], numd[nlist[i-1]:nlist[i], 0]))
                #zeff[j-1].append((np.trapz(numd[nlist[i-1]:nlist[i], j]*numd[nlist[i-1]:nlist[i], 0], numd[nlist[i-1]:nlist[i], 0]))/np.trapz(numd[nlist[i-1]:nlist[i], j], numd[nlist[i-1]:nlist[i], 0]))
                #print t_1, t_2, (np.trapz(numd[nlist[i-1]:nlist[i], j]*numd[nlist[i-1]:nlist[i], 0], numd[nlist[i-1]:nlist[i], 0]))
                #t_1 = (numd[nlist[i]-1, j]*numd[nlist[i]-1, 0]+numd[nlist[i], j]*numd[nlist[i], j])/2*dz-t_2

def print_nd(nacc):
    if args.sec == True:
        print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(nacc[0][0], nacc[1][0], nacc[2][0], sum(nacc[0][-2:]), sum(nacc[1][-2:]), sum(nacc[2][-2:]))
        #print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(nacc[0][1], nacc[1][1], nacc[2][1], nacc[0][-3], nacc[1][-3], nacc[2][-3])
        #print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(sum(nacc[0][:2]), sum(nacc[1][:2]), sum(nacc[2][:2]), sum(nacc[0][-3:]), sum(nacc[1][-3:]), sum(nacc[2][-3:]))
    else:
        print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(nacc[0][0], nacc[1][0], nacc[2][0], nacc[0][-1], nacc[1][-1], nacc[2][-1])
        #print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(nacc[0][1], nacc[1][1], nacc[2][1], nacc[0][-2], nacc[1][-2], nacc[2][-2])
        #print "%12.5f%12.5f%12.5f%12.5f%12.5f%12.5f" %(sum(nacc[0][:2]), sum(nacc[1][:2]), sum(nacc[2][:2]),  sum(nacc[0][-2:]), sum(nacc[1][-2:]), sum(nacc[2][-2:]))

def print_z(zeff):
    if args.sec == True:
        results = [zeff[0][0]-0.682, zeff[1][0]-0.682, zeff[2][0]-0.682, 6.364-zeff[0][-2]-0.682, 6.364-zeff[1][-2]-0.682, 6.364-zeff[2][-2]-0.682]
        #results = [zeff[0][1]-0.682, zeff[1][1]-0.682, zeff[2][1]-0.682, 6.364-zeff[0][-3]-0.682, 6.364-zeff[1][-3]-0.682, 6.364-zeff[2][-3]-0.682]
    else:
        results = [zeff[0][0]-0.682, zeff[1][0]-0.682, zeff[2][0]-0.682, 6.364-zeff[0][-1]-0.682, 6.364-zeff[1][-1]-0.682, 6.364-zeff[2][-1]-0.682]
        #results = [zeff[0][1]-0.682, zeff[1][1]-0.682, zeff[2][1]-0.682, 6.364-zeff[0][-2]-0.682, 6.364-zeff[1][-2]-0.682, 6.364-zeff[2][-2]-0.682]
    for i in results:
        if i <= 0:
            print "%12.5f" %0,
        elif i > 2:
...
print_nd(nacc)
#print zeff
#print "%12.5f%12.5f%12.5f" %(zlist[0], zlist[-2], zlist[-1])
