...
# hole radius. Build small unit first, and expand the pattern to larger plane.
...
if args.filename:
    ...
    filename = raw_input('Please input the filename of unitcell:\n')

if args.center:
    center = np.array(args.center)
else:
    center = np.array([float(i) for i in raw_input('Please input center coordinates:\n')])

if args.radius:
    radius = args.radius
else:
    radius = float(raw_input('Please input the radius of the hole:\n'))
