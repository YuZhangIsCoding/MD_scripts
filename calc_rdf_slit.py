                ...
                for i in xyz_res:
                    for j in xyz_ref:
                        if sum((i-j)**2) != 0:
                            ...
                            for subtemp, subdim in enumerate(dim_flat):
                                if temp[subdim] >= d_plane[subtemp]/2:
                                    temp[subdim] = d_plane[subtemp]-temp[subtemp]
...
plt.legend()
plt.savefig('RDF_slit.pdf')

fmt = ['%12.4f' for row in range(1+len(cyld[0]))]
np.savetxt('RDF_slit.txt', np.column_stack((distances, cyld)), fmt = fmt)
