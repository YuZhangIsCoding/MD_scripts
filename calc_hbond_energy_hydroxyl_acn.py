...
hdist = np.loadtxt('hbond_ha_dist_hn.txt')
...
#plt.plot(np.mean(hdist), np.mean(f(hdist)+g(hdist))/kt, 'o', ms = msize*2, mec = 'violet', mfc = 'violet', label = 'Average', alpha = 0.7)
plt.ylim([-100, 0])

pos = (np.mean(hdist), np.mean(f(hdist)+g(hdist))/kt)
plt.annotate('%.3f nm, %.2f kT' %pos, xy = pos, xytext = (0.25, -85), arrowprops = dict(arrowstyle="->", edgecolor = 'black', connectionstyle="arc3", shrinkB = 8), ha = 'center', va = 'bottom')
...
plt.savefig('Hbonds_energy_hydroxyl_ACN.pdf')
